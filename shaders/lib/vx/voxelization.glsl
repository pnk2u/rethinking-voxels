...
if (maxAvailableLod(center) >= 0) {
	...
	vec3 depthFromProjected =
	    vec3(vxPos[1][mostPerpendicularAxis] -
	             vxPos[0][mostPerpendicularAxis],
	         vxPos[2][mostPerpendicularAxis] -
	             vxPos[0][mostPerpendicularAxis],
	         vxPos[0][mostPerpendicularAxis]);
	...
	for (int lod = maxAvailableLod(center); lod >= 0; lod--) {
		ivec3 boundCoords[2] =
		    ivec3[2](vxPosToVxCoords(bounds[0], lod),
		             vxPosToVxCoords(bounds[1], lod));
		if (boundCoords[0] == ivec3(-1) ||
		    boundCoords[1] == ivec3(-1)) {
			continue;
		}
		...
		for (int x = projectionBoundCoords[0].x;
			...
			for (int y = projectionBoundCoords[0].y;
				...
				thisPos =
				    (thisPos - voxelVolumeSize * 0.5) / (1 << lod);
				...
				writeVoxelVolume(vxPosToVxCoords(thisPos, lod), lod,
				                 voxelData);
			}
		}
		firstValidLod = false;
